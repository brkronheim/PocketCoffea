...
def get_dijet(jets, taggerVars=True, remnant_jet = False):
    ...
    if remnant_jet:
        remnant_px = ak.sum(remnant.pt * np.cos(remnant.phi), axis=1)
        remnant_py = ak.sum(remnant.pt * np.sin(remnant.phi), axis=1)
        remnant_pz = ak.sum(remnant.pt * np.sinh(remnant.eta), axis=1)
        remnant_energy = ak.sum(
            np.sqrt(
                (remnant.pt * np.cosh(remnant.eta)) ** 2
                + remnant.mass ** 2
            ),
            axis=1,
        )
        remnant_pt = np.sqrt(remnant_px**2 + remnant_py**2)
        remnant_phi = np.arctan2(remnant_py, remnant_px)
        remnant_eta = np.arcsinh(
            remnant_pz / ak.where(remnant_pt != 0, remnant_pt, 1)
        )
        remnant_mass = np.sqrt(
            np.maximum(remnant_energy**2 - remnant_pt**2 - remnant_pz**2, 0)
        )
        remnant_charge = (
            ak.sum(remnant["charge"], axis=1)
            if "charge" in remnant.fields
            else ak.zeros_like(remnant_pt)
        )
        remnant_fields = {
            "pt": remnant_pt,
            "eta": remnant_eta,
            "phi": remnant_phi,
            "mass": remnant_mass,
            "charge": remnant_charge,
        }
        for var, value in remnant_fields.items():
